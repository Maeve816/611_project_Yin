...
setwd("/Users/yin816/Library/CloudStorage/OneDrive-UniversityofNorthCarolinaatChapelHill/Desktop/UNC CURRICULUM/2023 fall/611/611/6 Tidyverse & ggplot2")
df <- read_csv("/character-data.csv", col_types = cols(
...
## pivot_wider(gender_marital %>% group_by(character, universe, property_name) %>% summarize(value=value[1]), id_cols = character:universe, names_from="property_name", values_from="value")
```
...
## same as:    arrange(desc(n),property_name) %>% pull(property_name);
...
page_lengths <- read_csv("character-page-data.csv");
